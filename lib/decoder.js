...
Decoder.prototype._write = function(buf, enc, cb) {
  ...
  out: do {
    ...
    do {
      ...
      case STATE_TABLE_READ:
        state.pop()

        if (this.integer < staticTableLength)
          item = staticTable.table[this.integer]
        else
          item = this.table.table[this.integer - staticTableLength]
      ...
      case STATE_INDEXED:
        ...
        state[ state.length - 1 ] = STATE_EMIT
        state.push(STATE_TABLE_READ)
        break

      case STATE_LITERAL:
        state[ state.length - 1 ] = STATE_EMIT

        state.push(STATE_LITERAL_VALUE)
        state.push(STATE_STRING)

        if (this.integer) {
          this.state.push(STATE_TABLE_READ)
        } else {
